...
struct depth_t
{
    static inline constexpr double maxDepth = std::numeric_limits<double>::max();
...
depth_t scopeMin(const bbox &area, const byte* frame)
{
    ...
    for(int i = 0; i < area.w*area.h; i++)
    {
       ...
       pos = y*area.w + x;
       if(dep.depth > depth[pos] and depth[pos] > 0.0 and !std::isnan(depth[pos]))
       {
